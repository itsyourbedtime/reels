...
local function clear_track(tr)
  reel.clip[tr] = 1
  softcut.clear_range(reel.s[tr], clip_len_s * 48000)
...
local function load_clip(path)
  if path ~= "cancel" then
    if path:find(".aif") or path:find(".wav") then
      ...
      softcut.buffer_read_mono(path, 0, reel.s[trk], len/48000, 1, 1)
      reel.paths[trk] = path
      reel.clip[trk] = 1
